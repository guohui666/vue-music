    ...
    <div class="list-shortcut">
      <ul>
        <li v-for="(item,index) in shortcutList" :data-index="index" class="item"
            :class="{'current':currentIndex===index}" @touchstart="onShortcutTouchStart"
  ...
  export default {
    ...
    methods: {
      ...
      _scrollTo (index) {
        ...
        this.scrollY = -this.listHeight[parseInt(index)]
        this.$refs.listView.scroll.scrollToElement(this.$refs.listGroup[index], 0)
      },
      /*
        计算每个listGroup距离页面顶部的高度，clientHeight
       */
      _calculateHeight () {
        this.listHeight = []
    ...
    watch: {
      ...
      scrollY (newY) {
        ...
        for (let i = 0; i < listHeight.length; i++) {
          ...
          //  对比滚动事件派发后的高度区间，实时替换currentIndex
          //  点击右边入口为等于
